...
class MRConREDataset(data.Dataset):
    def __init__(self, path, rel2id, tokenizer, entpair_as_bag=False, bag_size=0, max_bag_size=0):
        ...
        # print("bag num: ", len(self.bag_scope))
        # max = -1
        # for i in self.bag_scope:
        #     if len(i) > max:
        #         max = len(i)
        # print(max)
        # self.count = [0 for i in range(max)]
        
        # for i in self.bag_scope:
        #     self.count[len(i)-1] += 1
        
        # for i in range(len(self.count)):
        #     print("bag size of {} has {}".format(i+1, self.count[i]))
...
def MRConREDataLoader(path, rel2id, tokenizer, batch_size, \
                shuffle, entpair_as_bag=False, bag_size=0, max_bag_size=0, \
                drop_last=False, num_workers=16, collate_fn=MRConREDataset.collate_fn):

    if bag_size == 0:
        collate_fn = MRConREDataset.collate_fn
    else:
        collate_fn = MRConREDataset.collate_bag_size_fn

    dataset = MRConREDataset(
        path=path,
        rel2id=rel2id,
        tokenizer=tokenizer,
